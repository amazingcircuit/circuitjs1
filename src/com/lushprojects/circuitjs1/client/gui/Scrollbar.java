...
public class Scrollbar extends  Composite implements 
	...
	public Scrollbar(int orientation, int value, int visible, int minimum, int maximum) {
		...
		can.setWidth((Gui.VERTICALPANELWIDTH)+" px");
		can.setHeight("40 px");
		can.setCoordinateSpaceWidth(Gui.VERTICALPANELWIDTH);
...
//	}
	...
	public void draw() {
		...
		g.setLineWidth(1.0);
		g.fillRect(0,0,Gui.VERTICALPANELWIDTH,SCROLLHEIGHT);
		g.beginPath();
		g.moveTo(HMARGIN+SCROLLHEIGHT-3, 0);
		g.lineTo(HMARGIN, SCROLLHEIGHT/2);
		g.lineTo(HMARGIN+SCROLLHEIGHT-3, SCROLLHEIGHT);
		g.moveTo(Gui.VERTICALPANELWIDTH-HMARGIN-SCROLLHEIGHT+3, 0);
		g.lineTo(Gui.VERTICALPANELWIDTH-HMARGIN, SCROLLHEIGHT/2);
		g.lineTo(Gui.VERTICALPANELWIDTH-HMARGIN-SCROLLHEIGHT+3, SCROLLHEIGHT);
		g.stroke();
		if (enabled)
		...
		g.lineTo(Gui.VERTICALPANELWIDTH-HMARGIN-SCROLLHEIGHT-BARMARGIN, SCROLLHEIGHT/2);
		g.stroke();
		double p=HMARGIN+SCROLLHEIGHT+BARMARGIN+((Gui.VERTICALPANELWIDTH-2*(HMARGIN+SCROLLHEIGHT+BARMARGIN))*((double)(val-min)))/(max-min);
		if (enabled) {
...
//			g.stroke();
	...
	int calcValueFromPos(int x){
		int v;
		v= min+(max-min)*(x-HMARGIN-SCROLLHEIGHT-BARMARGIN)/(Gui.VERTICALPANELWIDTH-2*(HMARGIN+SCROLLHEIGHT+BARMARGIN));
		if (v<min)
			v=min;
		if (v>max)
			v=max;
...
//		GWT.log("Down");
		...
		if (enabled){
			if (e.getX()<HMARGIN+SCROLLHEIGHT ) {
				...
				if (e.getX()>Gui.VERTICALPANELWIDTH-HMARGIN-SCROLLHEIGHT ) {
					if (val<max)
						val++;
				}
				else {
...
//		e.preventDefault();
		if (enabled && attachedElm!=null && attachedElm.isMouseElm())
			CircuitElm.sim.gui.setMouseElm(null);
		if (enabled && dragging) {
			val=calcValueFromPos(e.getX());	
			dragging=false;
			draw();
	...
	public void onMouseOver(MouseOverEvent e){
		...
		if (enabled && attachedElm!=null)
			 CircuitElm.sim.gui.setMouseElm(attachedElm);
	}
