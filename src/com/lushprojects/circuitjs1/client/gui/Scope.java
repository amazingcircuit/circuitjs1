...
//    Image image;
    ...
    public int rightEdge() { return rect.getX()+rect.getWidth(); }
 ...
 //   	if (plot2d && dpixels != null) {
    ...
    void drawTo(int x2, int y2) {
		...
		if (sim.gui.printableCheckItem.getState()) {
...
//    	}
    ...
    void clear2dView() {
    	if (imageContext!=null) {
    		...
    		imageContext.fillRect(0, 0, rect.getWidth()-1, rect.getHeight()-1);
    	}
    	draw_ox = draw_oy = -1;
    }
    ...
    void drawFFTVerticalGridLines(Graphics g) {
    	      ...
    	      for (int i = 0; i < divs; i++) {
    	        ...
    	        g.setColor("#FF0000");
    	        g.drawString(s, x + 2, rect.getHeight());
    	      }
    	    }
    	
    	    void drawFFT(Graphics g) {
    	      int y = (rect.getHeight() - 1) / 2;
    	      ...
    	      int prevHeight = (int) ((magnitude * y) / minMaxV);
    	      int prevX = 0;
    	      g.setColor("#FF0000");
    	      for (int i = 1; i < scopePointCount / 2; i++) {
    	        int x = 2 * i * rect.getWidth() / scopePointCount;
    	        ...
    	        int height = (int) ((magnitude * y) / minMaxV);
    ...
    void draw2d(Graphics g) {
    	if (imageContext==null)
    		return;
    	g.context.save();
    	g.context.translate(rect.getX(), rect.getY());
//   	int i;
...
//    	}
    	...
    	if (alphadiv>2) {
    		...
    		imageContext.fillRect(0,0,rect.getWidth(),rect.getHeight());
...
//    	g.drawImage(image, r.x, r.y, null);
    	...
    	int yt = 10;
    	int x = 0;
    	if (text != null &&  rect.getHeight() > yt+5) {
    		g.drawString(text, x, yt);
    		yt += 15;
    	}
    	g.setColor(Color.green);
    	g.drawLine(0, rect.getHeight()/2, rect.getWidth()-1, rect.getHeight()/2);
    	if (!plotXY)
    		g.setColor(Color.yellow);
    	g.drawLine(rect.getWidth()/2, 0, rect.getWidth()/2, rect.getHeight()-1);
    	g.context.restore();
    }
...
//    		pixels[i] = col;
    	...
    	if (sim.gui.scopeSelected == -1 && elm.isMouseElm())
...
//    		}
    	g.setColor(col);
    		g.drawLine(0,yl,rect.getWidth()-1,yl);
//    		for (i = 0; i != r.width; i++)
...
//    		gridStep *= 10;
    	...
    	g.setColor(col);
    		g.drawLine(gx,0,gx,rect.getHeight()-1);
...
//    	x += r.x;
    ...
    public MenuBar getMenu() {
    	if (elm == null)
    		return null;
    	if (elm instanceof TransistorElm) {
    		sim.gui.scopeIbMenuItem.setState(value == VAL_IB);
    		sim.gui.scopeIcMenuItem.setState(value == VAL_IC);
    		sim.gui.scopeIeMenuItem.setState(value == VAL_IE);
    		sim.gui.scopeVbeMenuItem.setState(value == VAL_VBE);
    		sim.gui.scopeVbcMenuItem.setState(value == VAL_VBC);
    		sim.gui.scopeVceMenuItem.setState(value == VAL_VCE && ivalue != VAL_IC);
    		sim.gui.scopeVceIcMenuItem.setState(value == VAL_VCE && ivalue == VAL_IC);
    		return sim.gui.transScopeMenuBar;
    	} else {
    		sim.gui.scopeVMenuItem    .setState(showV && value == 0);
    		sim.gui.scopeIMenuItem    .setState(showI && value == 0);
    		sim.gui.scopeScaleMenuItem.setState(showScale);
    		sim.gui.scopeMaxMenuItem  .setState(showMax);
    		sim.gui.scopeMinMenuItem  .setState(showMin);
    		sim.gui.scopeFreqMenuItem .setState(showFreq);
    		sim.gui.scopePowerMenuItem.setState(value == VAL_POWER);
    		sim.gui.scopeVIMenuItem   .setState(plot2d && !plotXY);
    		sim.gui.scopeXYMenuItem   .setState(plotXY);
    		sim.gui.scopeSelectYMenuItem.setEnabled(plotXY);
    		sim.gui.scopeFFTMenuItem.setState(showFFT);
    		sim.gui.scopeResistMenuItem.setState(value == VAL_R);
    		sim.gui.scopeResistMenuItem.setEnabled(elm instanceof MemristorElm);
    		return sim.gui.scopeMenuBar;
    	}
    }
    ...
    void allocImage() {
		if (imageCanvas != null) {
			imageCanvas.setWidth(rect.getWidth() + "PX");
			imageCanvas.setHeight(rect.getHeight() + "PX");
			imageCanvas.setCoordinateSpaceWidth(rect.getWidth());
			imageCanvas.setCoordinateSpaceHeight(rect.getHeight());
			clear2dView();
		}
    }
...
//    }
    ...
    public void handleMenu(String mi) {
    	if (mi == "showvoltage")
    		showVoltage(sim.gui.scopeVMenuItem.getState());
    	if (mi == "showcurrent")
    		showCurrent(sim.gui.scopeIMenuItem.getState());
    	if (mi=="showscale")
    		showScale(sim.gui.scopeScaleMenuItem.getState());
    	if (mi == "showpeak")
    		showMax(sim.gui.scopeMaxMenuItem.getState());
    	if (mi == "shownegpeak")
    		showMin(sim.gui.scopeMinMenuItem.getState());
    	if (mi == "showfreq")
    		showFreq(sim.gui.scopeFreqMenuItem.getState());
    	if (mi == "showfft")
    		showFFT(sim.gui.scopeFFTMenuItem.getState());
    	if (mi == "showpower")
    		setValue(VAL_POWER);
    	if (mi == "showib")
    		setValue(VAL_IB);
    	...
    	if (mi == "showvvsi") {
    		plot2d = sim.gui.scopeVIMenuItem.getState();
    		plotXY = false;
    		resetGraph();
    	}
    	if (mi == "plotxy") {
    		plotXY = plot2d = sim.gui.scopeXYMenuItem.getState();
    		if (yElm == null)
    			selectY();
    		resetGraph();
    	}
