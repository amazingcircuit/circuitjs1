...
import com.lushprojects.circuitjs1.client.gui.Scrollbar;
...
public class PotElm extends CircuitElm implements Command, MouseWheelHandler {
    ...
    void createSlider() {
    	sim.gui.addWidgetToVerticalPanel(label = new Label(sliderText));
    	label.addStyleName("topSpace");
    	int value = (int) (position*100);
    	sim.gui.addWidgetToVerticalPanel(slider = new Scrollbar(Scrollbar.HORIZONTAL, value, 1, 0, 101, this, this));
   ...
   // 	slider.addAdjustmentListener(this);
    ...
    public void delete() {
	sim.gui.removeWidgetFromVerticalPanel(label);
	sim.gui.removeWidgetFromVerticalPanel(slider);
    }
	...
	int offset = 0;
	if (abs(dx) > abs(dy)) {
	    dx = sim.gui.snapGrid(dx/2)*2;
	    point2.x = x2 = point1.x + dx;
	    offset = (dx < 0) ? dy : -dy;
	    point2.y = point1.y;
	} else {
	    dy = sim.gui.snapGrid(dy/2)*2;
	    point2.y = y2 = point1.y + dy;
	    offset = (dy > 0) ? dx : -dx;
	    point2.x = point1.x;
	}
	if (offset == 0)
	    offset = sim.gui.gridSize;
	...
	int i;
	int ox = 0;
	int hs = sim.gui.euroResistorCheckItem.getState() ? 6 : 8;
	double v1 = volts[0];
	double v2 = volts[1];
	double v3 = volts[2];
	...
	if (!sim.gui.euroResistorCheckItem.getState()) {
	    // draw zigzag
	...
	arr[2] = "R1 = " + getUnitText(resistance1, Gui.ohmString);
	arr[3] = "R2 = " + getUnitText(resistance2, Gui.ohmString);
